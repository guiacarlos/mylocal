/**
 * SynaxisCore — "ACIDE en el navegador". Base de datos JSON + despachador.
 *
 * Puntos de diseño:
 *  1. Contrato idéntico al dispatcher PHP: `execute({action, ...})` → {success, data, error}.
 *  2. CRUD con metadatos `_version`, `_createdAt`, `_updatedAt`.
 *  3. Flag `_REPLACE_` en `data` → reemplaza doc en vez de merge.
 *  4. Master collections (users/roles/projects/system_logs) van a DB
 *     IndexedDB separada para no contaminar el scope per-proyecto.
 *  5. Versionado interno en <collection>__versions (últimas 5).
 *  6. `oplog`: cada escritura se append-only en una colección dedicada
 *     pensando en Fase 3 (sync con server).
 */
...
export class SynaxisCore {
    ...
    /**
     * Normaliza un request que puede venir:
     *   - plano: { action, collection, id, data, params }
     *   - anidado: { action, data: { collection, id, data, params } }
     *   - plano con extras: { action, ...payload }
     * igual que el dispatcher PHP.
     */
    ...
    private async appendOp(op: OpType, collection: string, targetId: string, payload: unknown): Promise<void> {
        try {
            ...
            // El oplog no bloquea nunca la escritura principal.
        }
    }
