...
export function AgendaPage() {
    ...
    useEffect(() => {
        setLoading(true);
        listCitas(client, localId, base.toISOString(), addDays(until, 1).toISOString())
    ...
    async function handleCreate() {
        ...
        try {
            const c = await createCita(client, { local_id: localId, recurso_id: 'r_default', ...data });
        ...
        <div className="db-card">
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                <div>
                    ...
                    <div className="db-card-sub">{fmtDate(base)} — {fmtDate(until)}</div>
                </div>
                    ...
                    {days.map((d, i) => {
                        ...
                        return (
                            <div key={i} className={`cl-day-col${isToday(d) ? ' cl-day-today' : ''}`}>
                                ...
                                {dc.length === 0 && <div style={{ fontSize: 11, color: 'var(--cl-text-muted)' }}>Sin citas</div>}
                                {dc.map(c => (
