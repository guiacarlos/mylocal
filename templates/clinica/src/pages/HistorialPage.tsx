...
export function HistorialPage() {
    ...
    if (!paciente) return (
        <div className="db-card">
            <p style={{ color: 'var(--cl-text-muted)' }}>Paciente no encontrado.</p>
            <button className="db-btn db-btn--ghost" onClick={() => navigate('/pacientes')}><ArrowLeft size={14} /> Volver</button>
        </div>
    );
    ...
    return (
        <div className="db-card">
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                <button className="db-btn db-btn--ghost" onClick={() => navigate('/pacientes')}><ArrowLeft size={14} /></button>
                <div>
                    <div className="db-card-title" style={{ marginBottom: 0 }}>{paciente.nombre}</div>
                    <div style={{ fontSize: 13, color: 'var(--cl-text-muted)' }}>
                        {[paciente.telefono, paciente.email].filter(Boolean).join(' · ')}
                    </div>
                </div>
            </div>

            {paciente.notas && <p style={{ fontSize: 13, background: 'var(--cl-bg-soft)', padding: '8px 12px', borderRadius: 6, marginBottom: 16 }}>{paciente.notas}</p>}
            ...
            {interacciones.length === 0 && (
                <p style={{ color: 'var(--cl-text-muted)', fontSize: 13 }}>Sin historial. Añade la primera nota.</p>
            )}
