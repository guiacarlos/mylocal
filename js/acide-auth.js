    ...
    loginForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (result.success) {
                ...
                // Redirección — Academia (cursos) va a /academy. Todo lo operativo (admin incluido) va al TPV.
                // El TPV oculta los botones de ajustes a no-admins vía authService.isAdmin().
                setTimeout(() => {
                    const role = (user.role || '').toLowerCase();
                    if (['estudiante', 'cliente', 'client', 'standard', 'pro', 'premium'].includes(role)) {
                        window.location.href = '/academy';
                    } else {
                        // superadmin, admin, administrador, maestro, editor, sala, cocina, camarero → TPV
