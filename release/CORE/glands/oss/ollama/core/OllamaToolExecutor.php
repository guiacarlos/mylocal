...
/**
 * 🛠️ OllamaToolExecutor - Ejecutor de Herramientas MCP
 * 
...
class OllamaToolExecutor
{
    ...
    public function __construct($mcp, OllamaConfig $config)
    {
        $this->mcp = $mcp;
        $this->config = $config;
    }
     ...
     * @return string Resultado de la herramienta
     */
    public function execute(string $toolName, array $args): string
    {
        try {
            ...
            error_log("Ollama Tool Error [{$toolName}]: " . $e->getMessage());
