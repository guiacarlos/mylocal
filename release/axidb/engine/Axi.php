...
class Axi
{
    ...
    public function execute(array|Op\Operation $request): array
    {
        ...
        // 4.5 Audit Logging (Solo para operaciones autenticadas o fallidas)
        if ($user || !$isPublic) {
            $this->logAudit($user, $opName, $resource, $isPublic);
        }
    ...
    /**
     * Registro de auditoría soberana
     */
    private function logAudit(?array $user, string $action, ?string $resource, bool $isPublic): void
    {
        $logDir = DATA_ROOT . '/logs/audit';
        if (!is_dir($logDir)) {
            mkdir($logDir, 0755, true);
        }
        $logFile = $logDir . '/' . date('Y-m-d') . '.audit.log';
        
        $userName = $user ? ($user['email'] ?? ($user['id'] ?? 'user')) : 'ANONYMOUS';
        $role     = $user ? ($user['role'] ?? 'none') : 'none';
        $ip       = $_SERVER['REMOTE_ADDR'] ?? '0.0.0.0';
        $status   = ($user || $isPublic) ? 'GRANTED' : 'DENIED';

        $entry = sprintf(
            "[%s] [%s] IP:%s USER:%s ROLE:%s ACTION:%s RESOURCE:%s PUBLIC:%s\n",
            date('H:i:s'),
            $status,
            $ip,
            $userName,
