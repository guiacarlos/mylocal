    ...
    loginForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const response = await fetch('/acide/index.php', {
                ...
                credentials: 'include', //  SOBERANÍA: Enviar y recibir cookies HttpOnly
                body: JSON.stringify({
                    action: 'auth_login',
                    email: email,
                    password: password
            ...
            if (result.success) {
                ...
                if (user) {
                    ...
                    console.log(' [ACIDE] Usuario persistido:', user.email);
                }

                if (token) {
                    console.log(' [ACIDE] Token detectado y persistido.');
                } else {
                    console.log(' [ACIDE] Token ausente (Soberanía vía Cookie HttpOnly detectada).');
                }
