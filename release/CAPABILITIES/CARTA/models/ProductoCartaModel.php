...
class ProductoCartaModel
{
    ...
    public function create($data)
    {
        ...
        $doc = [
            ...
            'imagen_mejorada_url' => $data['imagen_mejorada_url'] ?? '',
            'alergenos' => $alergenos,
            'alergenos_sugeridos' => $this->validarAlergenos($data['alergenos_sugeridos'] ?? []),
            'alergenos_revisados' => !empty($data['alergenos_revisados']),
            'ingredientes' => is_array($data['ingredientes'] ?? null) ? $data['ingredientes'] : [],
            'texto_promocional' => $data['texto_promocional'] ?? '',
            'es_especialidad' => !empty($data['es_especialidad']),
            'iva_tipo' => $ivaTipo,
            'disponible' => $data['disponible'] ?? true,
            'orden' => intval($data['orden'] ?? 0),
            'origen_import' => $data['origen_import'] ?? 'manual',
            'created_at' => date('c'),
            'updated_at' => date('c')
        ];
    ...
    public function update($id, $data)
    {
        ...
        if (isset($data['alergenos_sugeridos'])) {
            $data['alergenos_sugeridos'] = $this->validarAlergenos($data['alergenos_sugeridos']);
        }
